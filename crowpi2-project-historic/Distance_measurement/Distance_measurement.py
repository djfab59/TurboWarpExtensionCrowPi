...
distancesensor = DistanceSensor(echo = 26,trigger = 16, max_distance = 5)

try:
    while True:
        print('Distance:',round(distancesensor.distance * 100, 2),"cm")
        lcd_screen.write_lcd(text="Distance:\n%scm" % round(distancesensor.distance * 100, 2))
        time.sleep(0.5)
        lcd_screen.clear()
#         lcd_screen.turn_off()
